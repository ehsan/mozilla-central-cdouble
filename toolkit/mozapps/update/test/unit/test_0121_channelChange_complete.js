...
/* Channel change complete MAR file background patch apply test */
...
function run_test() {
  ...
  let toBeDeletedDir = getTargetDirFile("tobedeleted", true);
  do_check_false(toBeDeletedDir.exists());
  ...
  // Sorting on Linux is different so skip this check for now.
  if (!IS_UNIX) {
    checkUpdateLogContents(LOG_COMPLETE_CC_SWITCH_SUCCESS);
  }
